...
Util.Validate = (function(){
  ...
  var publicReturn = {
    ...
    zeroValidate: function(args) {
      this.paraNumValidate(arguments, 1, Infinity);
      this.nullValidate(arguments);
      for(var i in arguments) {
        if(arguments[i] == 0)
          throw "ZeroValidate: The parameter can not be zero in function:" + /function\s+(\w+)/.exec(arguments.callee.caller)[1];
      }
    }
  };
  return publicReturn;
